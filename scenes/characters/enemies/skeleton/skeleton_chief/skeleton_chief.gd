...
@onready var damage_bar: Node2D = $BossDamageBar
...
func take_damage(amount: float):
	...
	if health <= 2:
		...
		current_player = null
		player_in_range = false
		DropManager.drop_for_enemy("skeleton_chief", global_position, get_parent())
		died.emit()
...
func _on_death_animation_finished():
	StatisticsManager.record_kill("skeleton_chief")
